...
def channel(layer, n_channel, batch=None):
    ...
    @handle_batch(batch)
    def inner(model):
        if layer[-16:] == "_layernorm_after": # ViT TODO: document this
            return -model(layer)[:, :, n_channel].mean()
        elif layer[-34:] == "attention_attention_output_context": # ViT TODO: document this
            attention_head_start = n_channel * 64
            attention_head_end = (n_channel + 1) * 64
            return -model(layer)[:, :, attention_head_start:attention_head_end].mean()
        else:
            return -model(layer)[:, n_channel].mean()
    return inner
