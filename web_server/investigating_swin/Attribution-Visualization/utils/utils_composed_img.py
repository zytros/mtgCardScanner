...
#My function to convert indices from pytorch to tensorflow
def convert_indices_pytorch_to_tf(indices, gather_axis):
    # Get the shape of the indices tensor
    shape = indices.shape

    # Create a tensor with the indices of each element
    all_indices = np.indices(shape)

    # Create a tensor with the locations to gather
    gather_locations = np.reshape(indices, -1)

    # Create a tensor with the indices to gather
    gather_indices = []
    for axis in range(len(shape)):
        if axis == gather_axis:
            gather_indices.append(gather_locations)
        else:
            gather_indices.append(np.reshape(all_indices[axis], -1))

    # Stack the indices to create a 2D tensorhow
...
def torch_gather(tobe_gathered_tensor, torch_version_indices, gather_axis):
  ...
  #gather_indices = convert_indices_pytorch_to_tf(torch_version_indices, gather_axis)
